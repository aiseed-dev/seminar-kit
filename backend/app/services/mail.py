...
def no_form() -> tuple[str, str]:
    """様式の添付が見つからない(未登録の送信者)への定型返信。"""
    subject = "【要確認】申込様式が見つかりません"
    body = """お申し込みのメールを受け取りましたが、申込様式(Excel ファイル)の
添付が確認できませんでした。

お手数ですが、講座ページから申込様式をダウンロードしてご記入のうえ、
このアドレスへ添付してお送りください。
